      ...
      try {
        ...
        if (res.ok) {
          ...
          if (isMounted) {
            ...
            // ✅ FIX: Only redirect when BOTH conditions are true:
            // 1. Status is NAFATH_VERIFIED (not just AWAITING_NAFATH with saved code)
            // 2. nafathAdminApproved is explicitly true (admin clicked approve button)
          ...
          <div className="border border-gray-200 rounded-2xl p-6 bg-slate-50/50 flex flex-col justify-between shadow-sm">
            {status === 'NAFATH_VERIFIED' ? (
              <div className="space-y-4">
                ...
                <div className="bg-emerald-50 rounded-xl p-4 text-right border border-emerald-100/70">
                  <p className="text-[11px] text-emerald-800 font-bold leading-relaxed">
                    💡 يرجى عدم إغلاق هذه الصفحة أو الرجوع للخلف. سيقوم النظام تلقائياً بنقلك بمجرد منح الموافقة من الأدمن.
                  </p>
                </div>
              </div>
            ) : (
              <div className="space-y-4">
                ...
                <div className="bg-emerald-50 rounded-xl p-4 text-right border border-emerald-100/70">
                  <p className="text-[11px] text-emerald-800 font-bold leading-relaxed">
                    💡 يرجى عدم إغلاق هذه الصفحة. بمجرد إتمام التوثيق وقبول الطلب عبر تطبيق نفاذ، سيتم نقلك تلقائياً للخطوة التالية.
                  </p>
                </div>
              </div>
            )}
